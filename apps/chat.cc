...
public:
  ...
  // Helper to access the underlying UDP socket FD for polling
  FileDescriptor& adapter_fd() { return _datagram_adapter.fd(); }

private:
  Address virtual_src_;
};

void run_chat_ui( TCP_over_IP_over_Ethernet_over_UDP_Socket& sock )
{
  cout << CYAN << "============================================" << RESET << endl;
  cout << CYAN << "   Welcome to CS144 Chat Room! (Stable)" << RESET << endl;
  cout << CYAN << "============================================" << RESET << endl;
  cout << "Type a message and press Enter to send.\n" << endl;

  std::atomic<bool> connection_alive { true };
  std::thread reader_thread( [&sock, &connection_alive]() {
    try {
      while ( connection_alive ) {
        ...
        if ( !inbound.empty() ) {
          cout << GREEN << "[Peer]: " << RESET << inbound << flush;
        }
      }
    } catch ( ... ) {
      connection_alive = false;
  ...
  while ( connection_alive ) {
    ...
    if ( !connection_alive ) {
      cout << RED << "\n[System]: Connection closed." << RESET << endl;
      break;
    }
