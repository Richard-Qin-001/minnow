...
void Reassembler::flush()
{
  ...
  while ( !unassembled_strings_.empty() && unassembled_strings_.begin()->first == first_unassembled_index_ ) {
    ...
    const uint64_t sz = node->second.size();
...
void Reassembler::insert( uint64_t first_index, string data, bool is_last_substring )
{
  ...
  // current data bound
  const uint64_t left_bound_ = first_unassembled_index_;
  const uint64_t capacity = output_.writer().available_capacity();
  const uint64_t right_bound_ = first_unassembled_index_ + capacity;
  // cut off the overflow parts
  if ( first_index < left_bound_ ) {
    const uint64_t offset = left_bound_ - first_index;
    if ( offset >= data.size() ) {
      return;
    }
  ...
  while ( it != unassembled_strings_.end() && it->first <= end_index ) {
    ...
    const uint64_t cur_first = it->first;
    ...
    if ( cur_first < first_index ) {
      data.insert( 0, it->second.substr( 0, first_index - cur_first ) );
      first_index = cur_first;
    }
    // Handling right overflow: Current fragment ends later
    if ( cur_end > end_index ) {
      data += it->second.substr( end_index - cur_first );
      end_index = cur_end;
    }
    // erase the old one
    it = unassembled_strings_.erase( it );
  ...
  if ( !data.empty() ) {
    unassembled_strings_[first_index] = std::move( data );
  }
